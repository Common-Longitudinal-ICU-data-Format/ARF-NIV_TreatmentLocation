...
```{r}
model_equation <- paste0("death_hospice", " ~ icu_admission + ", paste(covariates, collapse = " + "))

model_data <- final_cohort |> 
    filter(triage_location %in% c("icu", "ward")) |>
    mutate(icu_admission = ifelse(tolower(triage_location)=="icu", 1, 0)) |>
    filter(first_hospital_id == "JHH")
...
# Run logistic regression
      ...
      marginal_effects_2 <- avg_predictions(
        model=model_1,
        newdata = datagrid(
          model=model_1,
          grid_type="counterfactual",
...
```{r}
# test_model_data <- final_cohort |>
#   filter(triage_location %in% c("icu", "ward")) 
# 
# # Set the cutoff for length of stay
# max_days_los <- ceiling(max(test_model_data$los_penalized))
# 
...
#   mutate(icu_admission = ifelse(tolower(triage_location)=="icu", 1, 0)) |>
#   ### Hospital specific ###
#   filter(first_hospital_id == "BMC")
# 
# model_equation <- paste0("los_ordinal ~ icu_admission + ", paste(covariates, collapse = " + "))
# 
# test_model <- clm(
#   model_equation,
...
# )
# 
# test_marginal_effects <- avg_predictions(test_model, by="icu_admission")
# 
# # p_3 <- avg_predictions(test_model,
# #                        newdata=datagrid(
# #                          model=test_model,
# #                           grid_type="counterfactual",
# #                           icu_admission=c(0,1),
