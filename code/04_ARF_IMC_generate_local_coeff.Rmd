---
title: "04_ARF_IMC_generate_local_coeff"
...
```{r}
write_csv(model_coefficients, paste0(model_out_dir, site,"_logit_model_coefficients.csv"))
write_csv(no_pandemic_model_coefficients, paste0(model_out_dir, site,"_no_covid_logit_model_coefficients.csv"))
```
  ...
  all_model_data <- all_model_data |>
    ...
    for(outcome in outcomes_counts){
      ...
      for(i in seq_len(nrow(hosp_data))){
        ...
        # Only do marginal effects for resp support free days
        if(outcome == "resp_support_free_days"){
          
          cat(paste0(" Running marginal effects...   "))
        
          # Running marginal effects, stratified by ICU admission
          marginal_effects <- avg_predictions(model_2, by="icu_admission")
          
          cat(paste0(" Saving marginal effects...   "))
          # Output marginal effects result, file name determined by cohort
          prefix_name <- ""
          # other option is the no covid group
          if(deparse(substitute(cohort_data)) == "no_pandemic_cohort"){
            prefix_name <- "_no_covid"
          }
          write_csv(marginal_effects, paste0(model_out_dir, site, 
                                             prefix_name, "_",outcome,
                                             "_", hosp_data$first_hospital_id[i],
                                             "_marginal_effects.csv"))
        }
        else{
          cat(paste0(" Not running marginal effects...   "))
        }
...
```{r}
write_csv(model_coefficients_prop_odds, paste0(model_out_dir,
                                               site,
                                               "_prop_odds_model_coefficients.csv"))

write_csv(no_pandemic_model_coefficients_prop_odds, paste0(model_out_dir, 
                                                        site,
                                                        "_no_covid_prop_odds_model_coefficients.csv"))
```


```{r}
# process outcome, triage to ICU
...
```{r}
model_coefficients_icu_adm <- run_secondary_models(final_cohort, hospital_data)
no_pandemic_model_coefficients_icu_adm <- run_secondary_models(no_pandemic_cohort, hospital_data)
```

```{r}
write_csv(model_coefficients_icu_adm, paste0(model_out_dir, site,"_icu_adm_model_coefficients.csv"))
write_csv(no_pandemic_model_coefficients_icu_adm, paste0(model_out_dir, site,"_no_covid_icu_adm_model_coefficients.csv"))
```