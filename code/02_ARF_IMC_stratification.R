...
{ # Setup
  ...
  { # Load config to specify local paths
    ...
    NIV_FREE_NAMES <- global_config$niv_free_devices
  ...
  { # Load the Required CLIF Tables
    ...
    true_tables <- c("patient", "hospitalization", "respiratory_support", "vitals", "labs","medication_admin_continuous", "patient_assessments", "adt", "hospital_diagnosis")
  ...
  { # Merge hospital blocks for all relevant CLIF tables
    ...
    cat("Add hospital blocks to CLIF tables\n")
    ...
    cat("     > clif_hospital_diagnosis...\n")
    clif_hospital_diagnosis <- clif_hospital_diagnosis |>
      # ensure same datatype
      mutate(hospitalization_id = as.character(hospitalization_id)) |>
      # mutate(hospitalization_id = cast(hospitalization_id, arrow::uint64()),
      #        hospitalization_id = cast(hospitalization_id, arrow::utf8())) |>
      inner_join(hospital_block_key_obj, by = c("hospitalization_id")) |>
      compute()
  } # Merge hospital blocks for all relevant CLIF tables
...
{ # Merge hospital blocks with patient information
  ...
  cat("Merge hospital blocks with patient information\n")
...
{ # Calculating Elixhauser Comorbidity Scores
  ...
  cat("Calculating Elixhauser Comorbidity Scores\n")
...
{ # Determining who is on NIV on admission (when leaving the ED)
  ...
  cat("Determining who is on NIV on admission\n")
...
{ # Determining physiologic phenotype of ARF
  ...
  cat("Determining physiologic phenotype of ARF\n")
  ...
  { # Hypoxics
    ...
    fio2_values <- fio2_values |>
      # Order by time within a given hospital block
      arrange(hospital_block_id, recorded_dttm) |>
      group_by(hospital_block_id) |>
      ...
      # Order by time within a given hospital block
      arrange(hospital_block_id, recorded_dttm) |>
      # Group by hospital block and a given device with no devices in beween
...
{ # Determining severity using non-respiratory SOFA
  ...
  cat("Determining severity using non-respiratory SOFA\n")
...
{ # Join with sepsis in ED indicator
  
  cat("Join with sepsis indicators\n")
...
{ # Determining number of days off NIV/more invasive in first DAYS_OFF_CUTOFF days from starting NIV
  ...
  cat("Number of days off NIV or more\n")
  ...
  all_resp_events <- clif_respiratory_support |>
    ...
    select(hospital_block_id, niv_start, recorded_dttm, device_category, on_niv_or_worse) |>
    collect()|>
  ...
  support_free_days <- all_resp_events |>
    arrange(hospital_block_id, recorded_dttm) |>
    group_by(hospital_block_id) |>
    summarise(
      ...
      # last time the patient was on NIV-or-worse (1); NA if never 1
      last_resp_support_dttm = if (any(on_niv_or_worse == 1)) max(recorded_dttm[on_niv_or_worse == 1]) else as.POSIXct(NA),
      
      # earliest time free (0) after the last 1 (terminal 0 start); NA if none
      resp_support_free_dttm = if (is.na(last_resp_support_dttm)) {
        if (any(on_niv_or_worse == 0)) min(recorded_dttm[on_niv_or_worse == 0]) else as.POSIXct(NA)
      } else {
        x <- recorded_dttm[on_niv_or_worse == 0 & recorded_dttm > last_resp_support_dttm]
        if (length(x) == 0) as.POSIXct(NA) else min(x)
      },
      
      # Calculate number of NIV free days (does NOT penalize for death/hospice yet)
      resp_support_free_days = if_else(
        # If there is never a terminal NIV free day
        is.na(resp_support_free_dttm),
        0,
        as.numeric(difftime(window_end, resp_support_free_dttm, units = "days"))
      ),
  ...
  final_cohort <- final_cohort |>
    mutate(
      ...
      # Penalize to 0 days if patient dies/discharges to hospice before 28 days
      resp_support_free_days = 
        ifelse(
          (in_hosp_death == 1 | hospice == 1) & 
            as.numeric(difftime(block_end_discharge, niv_start, units="days")) < DAYS_OFF_CUTOFF, 
          0, 
          resp_support_free_days
        )
    )
...
{ # Determining if patient was transferred to higher level of care
  ...
  cat("Transferred to higher level of care\n")
...
{ # Cleaning data, penalization
  ...
  # Calculate LOS penalized for death/hospice
  final_cohort <- final_cohort |>
...
{ # Save encounters
  ...
  # Save one encounter per patient with stratification
