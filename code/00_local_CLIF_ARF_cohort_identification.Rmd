---
title: "00_local_CLIF_ARF_cohort_identification"
...
true_tables <- c("hospitalization", "adt", "respiratory_support", "medication_admin_continuous", "code_status")
...
STUDY_START <- as.POSIXct("2018-01-01", tz=site_time_zone) # Start time
...
PRESSOR_LIST <- c("norepinephrine", "epinephrine","phenylephrine", "vasopressin","dopamine", "angiotensin", "dobutamine") # List of pressors that exclude patients
...
hospital_blocks <- hospitalization_information[, .(
  ...
  discharge_location = discharge_name[which.max(end_ts)]
...
write_log <- function(message, new_file=FALSE) {
  
  cat(message)
  
  file_name <- paste0(project_location, "/", site, "_project_output/cohort_size_", site,".txt")
...
write_log(message=paste0("Cohort size log for ", site,"\n"), new_file=TRUE)


```
## Grouping ED blocks
```{r Grouping ED blocks}
# Collect all encounters with above criteria
...
# Add the first and last hospital information
final_cohort[, `:=` (
...
final_cohort[, triage_location := ifelse(is.na(triage_idx), "discharge", location_category[triage_idx]), 
             by = hospital_block_id]
...
write_log(message=paste0("n = ", nrow(final_cohort), " for adult ED encounter blocks within the specified date range\n"), 
          new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after selecting only for ED visits where NIV has started in the ED block\n"), 
          new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after selecting only for NIV at least ", (STABILITY_WINDOW_MINS-MAX_NIV_BREAK_MINS)/60, " hours for the first ", (STABILITY_WINDOW_MINS/60)," hours after start of NIV (i.e., cumulative breaks no longer than ", MAX_NIV_BREAK_MINS/60," hours)\n"), 
          new_file=FALSE)
```

## Exclusion Criteria: Any IMV or tracheostomy in the ED
```{r Exclusion: Any IMV or tracheostomy in the ED}
# Identify any ED events where IMV occurred
imv_trach_events <- arrow::arrow_table(final_cohort) |>
  # Link ED events to any respiratory support event
  inner_join(
    clif_respiratory_support |>
      transmute(hospital_block_id, recorded_dttm, dev=device_category, tracheostomy),
    by = "hospital_block_id"
  ) |>
  # Select only respiratory support events that occurred in the ED and were IMV
  filter(
    tolower(dev) == "imv" | tracheostomy == 1,
    recorded_dttm >= start_ed,
    recorded_dttm <= stop_ed
  ) |>
  # Keeping only relevant rows to antijoin later
  select(hospital_block_id) |>
  distinct() |>
  collect()

# Removing all ED events where there was IMV
final_cohort <- final_cohort |>
  anti_join(imv_trach_events, by ="hospital_block_id")

# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after excluding all IMV and tracheostomy that was started in/predated the ED\n"), 
          new_file=FALSE)
```
...
final_cohort <- final_cohort |>
  # Remove patients who are still admitted/have no discharge date
  filter(!is.na(block_end_discharge)) |>
  # Set discharge location to unknown if NA
  mutate(discharge_location=coalesce(discharge_location, "Unknown"))

# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after excluding all patients who have no final discharge date\n"), 
          new_file=FALSE)

```
## Exclusion Criteria: Discharged to another hospital without admission
```{r Exclusion: Discharged to another hospital without admission}

# a <- final_cohort |>
#   filter(triage_location=="discharge")
#   #filter(discharge_location == "")
# 
# # Output to log file and view cohort size
# write_log(message=paste0("n = ", nrow(final_cohort), " after excluding all patients who were never admitted and were discharged directly from the ED to another hospital\n"), 
#           new_file=FALSE)

```


## Exclusion Criteria: Continous pressors needed in the ED
```{r Exclusion Criteria: Continous pressors needed in the ED}
...
# Remove all hospital blocks where patient received pressor in ED
final_cohort <- final_cohort |>
  anti_join(pressor_events, by ="hospital_block_id")

# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after excluding all continuous pressor use in the ED\n"), 
          new_file=FALSE)
```
...
# Identify patients who have "allow natural death" code status at time of leaving ED
allow_natural_death <- arrow::arrow_table(final_cohort) |>
  ...
  slice_head(n = 1) |>
  ungroup() |>
  # Selecting only patinets who have selected allow natural death as code status
  filter(tolower(code_status_category) == "and") |>
  # Select only list of hospitalization IDs to remove from final cohort
  select(hospital_block_id) |>
  # Filter to unique hospital block IDs
  distinct(hospital_block_id)
...
final_cohort <- final_cohort |>
  anti_join(allow_natural_death, by ="hospital_block_id")

# Output to log file and view cohort size
write_log(message=paste0("n = ", nrow(final_cohort), " after removing patients who have 'allow natural death' as code status at time leaving the ED\n"),
          new_file=FALSE)
```

# Output files
```{r Output final cohort into csv}
write_csv(final_cohort, file.path(paste0(project_location, "/private_tables"), "final_cohort.csv"))
```
