...
tables <- c("patient", "hospitalization", "vitals", "labs", 
            ...
            "therapy_details", "microbiology_culture", "sensitivity", "microbiology_nonculture", "code_status", "hospital_diagnosis")

# Tables that should be set to TRUE for this project
true_tables <- c("hospitalization", "adt", "respiratory_support", "medication_admin_continuous", "code_status", "hospital_diagnosis")
...
write_log <- function(n_curr=NA, message, new_file=FALSE) {
  ...
  file_name <- paste0(project_location, "/", site, "_project_output/broad_cohort_size_", site,".txt")
  
  if(new_file){
    file_path <- file(file_name, "wt")
    # Initialize previous n as NA
    n_prior <<- NA
  }
  else{
    file_path <- file(file_name, "at")
    if(is.na(n_prior)){
      message = paste0("n = ", n_curr, " after ", message)
    }
    else{
...
write_log(message=paste0("Cohort size log for ", site), new_file=TRUE)
...
write_log(n_curr=nrow(final_cohort), message="selecting for adult ED encounter blocks within the specified date range", new_file=FALSE)
...
# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="selecting only for ED visits where NIV has started in the ED block", new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message=paste0("selecting only for NIV at least ", (STABILITY_WINDOW_MINS-MAX_NIV_BREAK_MINS)/60, " hours for the first ", (STABILITY_WINDOW_MINS/60)," hours after start of NIV (i.e., cumulative breaks no longer than ", MAX_NIV_BREAK_MINS/60," hours)"), new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="excluding all IMV and tracheostomy that was started in/predated the ED", new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="excluding all patients who are still admitted", new_file=FALSE)
...
# Remove patients who are discharged to another hopsital without ever getting admitted here
final_cohort <- final_cohort |>
  filter(!(tolower(discharge_location) == "acute care hospital" & tolower(triage_location) == "discharge")) 

# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="excluding all patients who were discharged to an OSH wihtout ever being admitted to this hospital system", new_file=FALSE)
```
...
# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="removing patients who have 'allow natural death' as code status at time leaving the ED", new_file=FALSE)
```

## Exclusion Criteria: Discharged directly from ED
```{r Exclusion Criteria: Discharged directly from ED}

# Identify encounters where patients are discharged without any admission
ed_discharged_cohort <- final_cohort |>
  filter(tolower(triage_location) == "discharge")

# Remove these encounters from the final cohort
final_cohort <- final_cohort |>
  anti_join(ed_discharged_cohort |>
              select(hospital_block_id),
            by="hospital_block_id")

# Output to log file and view cohort size
write_log(n_curr=nrow(final_cohort), message="excluding all patients who were never admitted", new_file=FALSE)

```


# Output files defining broad cohort
```{r Output final cohort into csv}
# Send to csv
# Patients who were immediately discharged
write_csv(ed_discharged_cohort, file.path(paste0(project_location, "/private_tables"), "ed_discharged_cohort.csv"))
# Broad cohort
write_csv(final_cohort, file.path(paste0(project_location, "/private_tables"), "broad_cohort.csv"))
```
