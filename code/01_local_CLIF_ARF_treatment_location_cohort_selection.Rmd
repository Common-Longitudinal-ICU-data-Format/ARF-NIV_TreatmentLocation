---
title: "01_local_CLIF_ARF_treatment_location_cohort_selection"
...
# Cohort information
final_cohort <- read_csv(paste0(project_location, "/private_tables/broad_cohort.csv"), show_col_types=FALSE)
...
# Outlier thresholds
outlier_thresholds <- read_csv(paste0(project_location, "/project_outlier_thresholds.csv"), show_col_types=FALSE)
```
...
NIV_FREE_NAMES <- c("nasal cannula", "room air") # Device names that would be considered "off NIV"; "face mask" removed
...
SF_RATIO_CUTOFF <- 300 # SF ratio for pure hypoxemic
...
# Cutoffs for inclusion and exclusion criteria for analytic cohort
MIN_SF_INCLUSION <- 150
MIN_PF_INCLUSION <- NA
MAX_PACO2_INCLUSION <- 60
MIN_PH_INCLUSION <- 7.25
MIN_GCS_INCLUSION <- 9

# Cutoff for describing hospital as IMC capable
IMC_CAPABLE_CUTOFF <- 0.05 # 5%
```
...
# Flag patients with pressor use in the ED (will be exclusion criteria later)
```{r Flag continous pressors needed in the ED}

# Create list of all hospital block IDs where continuous pressors were started before leaving the ED
pressor_events <- arrow::arrow_table(final_cohort) |>
  select(hospital_block_id, start_ed, stop_ed) |>
  inner_join(clif_medication_admin_continuous |>
              # Select only relevant pressors
              filter(med_category %in% PRESSOR_LIST) |>
              # Select only relevant variables
              select(hospital_block_id, dttm=admin_dttm,
                     med_category, dose=med_dose, mar_action_name),
           by = "hospital_block_id")|>
  # Select only meds from when patient was in the ED
  filter(
    dttm >= start_ed,
    dttm <= stop_ed,
  ...
  filter(!is.na(dose) & !is.nan(dose) & dose > 0) |>
  # Select only hospital block IDs
  select(hospital_block_id) |>
  # Filter to unique hospital block IDs
  distinct(hospital_block_id) |>
  # Create a flag for these pressor events
  mutate(pressor_in_ed = TRUE) |>
  # Load in data
  collect()

# Flag all hospital blocks where patient received pressor in ED
final_cohort <- final_cohort |>
  left_join(pressor_events, by ="hospital_block_id") |>
  # Set flag as false (no pressor in ED) if it did not exist in pressor events
  mutate(pressor_in_ed = coalesce(pressor_in_ed, FALSE))
```
...
elixhauser_scores <- rbind(get_elixhauser_flags(clif_hospital_diagnosis, 9), get_elixhauser_flags(clif_hospital_diagnosis, 10)) |>
  ...
  mutate(elixhauser_total = 
           ...
           diabunc * 0 + # Uncomplicated diabetes
           diabc * 0 + # Complicated diabetes
...
final_cohort <- final_cohort |>
  left_join(elixhauser_scores |> 
              select(hospital_block_id, elixhauser_total),
            by = "hospital_block_id") |>
  # For those with no comorbidities (score would be NA), fill in with default score of 0
  mutate(elixhauser_total = replace_na(elixhauser_total, 0))
```
...
# Add the last start time for respiratory device to final cohort
final_cohort <- final_cohort |>
  left_join(
    last_resp_device |>
...
# was NIPPV, CPAP, etc. that they go to their discharge location (e.g., home, acute care hospital, etc.)
...
# Determine penalization variables (e.g., death, hospice)
final_cohort <- final_cohort |>
  mutate(
...
final_cohort <- final_cohort |>
  mutate(
    # Calculate length of NIV +- IMV, no penalization
    days_of_respiratory_support = as.numeric(difftime(niv_stop, niv_start, units="days")),
    # Calculate the amount of time off NIV in first DAYS_OFF_CUTOFF days from starting NIV
    days_off = case_when(
      is.na(days_of_respiratory_support) ~ 0, # Patients who never stop NIV/more invasive
      days_of_respiratory_support >= DAYS_OFF_CUTOFF ~ 0, # Patients on NIV/more invasive for DAYS_OFF_CUTOFF days or longer
      in_hosp_death == 1 | hospice == 1 ~ 0, # Patients who died in hospital or were discharged to hospice
      TRUE ~ (DAYS_OFF_CUTOFF - days_of_respiratory_support) # All other patients who stop NIV/more invasive within DAYS_OFF_CUTOFF days
    )
    )
...
final_cohort <- final_cohort |>
  mutate(
    ...
    # Store race
    race_summary = factor(ifelse(tolower(race_category) == "white" | 
    ...
    # Factorize sex
    sex_category = coalesce(sex_category, "Unknown"),
    sex_category = factor(sex_category, levels = c("Male", "Female", "Unknown")),
    # Factorize ethnicity
    ethnicity_category = factor(ethnicity_category, levels = c("Hispanic", "Non-Hispanic")),
...
```{r Categorizing hospitals as IMC capable}
...
# Save this broad cohort before identifying analytic cohort
```{r Save broad cohort}
write_csv(final_cohort, file.path(paste0(project_location, "/private_tables"), "broad_cohort_with_stratification.csv"))

# Set this as the baseline of how many entries
n_prior <- nrow(final_cohort)
```

## Creating file to output cohort sizes as we exclude
```{r Creating cohort size log file}

write_log <- function(n_curr=NA, message, new_file=FALSE) {
  
  file_name <- paste0(project_location, "/", site, "_project_output/analytic_cohort_size_", site,".txt")
  
  if(new_file){
    file_path <- file(file_name, "wt")
  }
  else{
    file_path <- file(file_name, "at")
    
    message=paste0("n = ", n_curr, " after ", message, " (n = ",(n_prior-n_curr)," (", round(100*(n_prior-n_curr)/n_prior),"%) removed)")
    n_prior <<- n_curr
  }
  writeLines(paste0(message, "\n"), file_path)
  cat(message)
  close(file_path)
}

# Open file in write-text mode
write_log(message=paste0("Analytic cohort size log for ", site), new_file=TRUE)


```


# Identify Analytic Cohort
```{r Identify Analytic Cohort}

# Exclusion criteria: No continuous pressors in the ED
analytic_cohort <- final_cohort |>
  filter(pressor_in_ed == FALSE)

write_log(nrow(analytic_cohort), "removing patients on continuous pressors in the ED")

# Exclusion criteria: Worst SF < inclusion cutoff
analytic_cohort <- analytic_cohort |>
  # Keep NAs
  filter(is.na(sf_value) | sf_value >= MIN_SF_INCLUSION)

write_log(nrow(analytic_cohort), paste0("removing patients with SF worse than ", MIN_SF_INCLUSION))

if(!is.na(MIN_PF_INCLUSION))
{
  # Exclusion criteria: Worst PF < inclusion cutoff
  analytic_cohort <- analytic_cohort |>
    # Keep NAs
    filter(is.na(pf_value) | pf_value >= MIN_PF_INCLUSION)
  
  write_log(nrow(analytic_cohort), paste0("removing patients with PF worse than ", MIN_PF_INCLUSION))
}

# Exclusion criteria: Worst PaCO2 > inclusion cutoff AND Worst pH < inclusion cutoff
analytic_cohort <- analytic_cohort |>
  # Keep NAs
  filter(is.na(pco2_value) | is.na(ph_value) | 
           pco2_value <= MAX_PACO2_INCLUSION |
           ph_value >= MIN_PH_INCLUSION)

write_log(nrow(analytic_cohort), paste0("removing patients with PaCO2 worse than ", MAX_PACO2_INCLUSION, " AND worst pH is worse than ", MIN_PH_INCLUSION))


# Exclusion criteria: Last GCS < inclusion cutoff
analytic_cohort <- analytic_cohort |>
  # Keep NAs
  filter(is.na(last_gcs) | last_gcs >= MIN_GCS_INCLUSION)

write_log(nrow(analytic_cohort), paste0("removing patients with GCS worse than ", MIN_GCS_INCLUSION))

analytic_cohort <- analytic_cohort |>
  # Remove cases where patient presented to a non "IMC capable" hospital
  filter(imc_capable == 1)

write_log(nrow(analytic_cohort), paste0("removing patients who presented to hospitals without IMC capability"))

# Exclusion criteria: Missing SF ratio
analytic_cohort <- analytic_cohort |>
  # Remove cases where  SF is NA
  filter(!(is.na(sf_value)))

write_log(nrow(analytic_cohort), "removing patients without valid SF value during stability window")

# Exclusion criteria: Missing pH 
analytic_cohort <- analytic_cohort |>
  # Remove cases where  pH is NA
  filter(!(is.na(ph_value)))

write_log(nrow(analytic_cohort), "removing patients without valid pH value during stability window")

# Commented out because these patients were removed in broad cohort
# # Exclusion criteria: Direct discharge
# analytic_cohort <- analytic_cohort |>
#   filter(triage_location != "discharge")
# 
# write_log(nrow(analytic_cohort), "removing patients who are never admitted (i.e., triage location is discharge)")

```

# Output analytic cohort
```{r Output analytic cohort into csv}
write_csv(analytic_cohort, file.path(paste0(project_location, "/private_tables"), "analytic_cohort_with_stratification.csv"))
```
