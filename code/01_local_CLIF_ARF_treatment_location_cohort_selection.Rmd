...
MIN_SF_INCLUSION <- 148
MIN_PF_INCLUSION <- 100
...
clif_patient_assessments <- clif_patient_assessments |>
  ...
  mutate(hospitalization_id = arrow::cast(hospitalization_id, arrow::float64())) |>
  inner_join(hospital_block_key_obj, by = c("hospitalization_id")) |>
  compute()
...
# Create function to determine elixhauser flags for a given diagnosis code format (e.g., ICD 9 or 10)
...
# These ones will not add to 100%
final_cohort <- final_cohort |>
  mutate(
    # Define patients as hypoxic if meeting hypoxic criteria
    is_hypoxic = ifelse(sf_value <= SF_RATIO_CUTOFF | pf_value <= PF_RATIO_CUTOFF, 1, 
                             ifelse(sf_value > SF_RATIO_CUTOFF | pf_value > PF_RATIO_CUTOFF, 0, NA_integer_)),
