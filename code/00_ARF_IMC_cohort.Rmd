...
NIV_NAMES <- c("nippv", "high flow nc") # Device names that are included as NIV
...
# Identify all events where pressors were used in the ED
...
# Remove these cases from the final cohort
final_cohort <- final_cohort |>
  anti_join(pressor_events, 
            by = "hospital_block_id"
  )
...
crrt_events <- arrow::arrow_table(final_cohort) |>
  select(hospital_block_id, start_ed, stop_ed) |>
  inner_join(clif_crrt_therapy, by = "hospital_block_id") |>
  # Select only crrt from when patient was in the ED
  filter(
    recorded_dttm >= start_ed,
    recorded_dttm <= stop_ed
  ) |>
